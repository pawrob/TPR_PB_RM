                ...
                foreach (XmlNode child in node.ChildNodes)
                {
                    ...
                    switch (node.Name)
                    {
                        ...
                        case "warehouse":
                            ...
                            break;
                        case "factures":
                            dataContext.SoldCars.Add(new SellCar(dataContext.Clients.Find(c => c.Id.Equals(Guid.Parse(elements["client"]))), dataContext.WarehouseItems.Find(c => c.Id.Equals(Guid.Parse(elements["warehouseItem"]))), Guid.Parse(elements["id"]), DateTime.ParseExact(elements["boughtTime"], "dd.MM.yyyy", CultureInfo.InvariantCulture)));
                            break;
                        case "billesOfSale":
                            dataContext.BoughtCars.Add(new BuyCar( dataContext.WarehouseItems.Find(c => c.Id.Equals(Guid.Parse(elements["warehouseItem"]))), Guid.Parse(elements["id"]), DateTime.ParseExact(elements["boughtTime"], "dd.MM.yyyy", CultureInfo.InvariantCulture)));
                            break;
                    }
                }
            }
